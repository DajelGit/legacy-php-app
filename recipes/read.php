...
$retrieveRecipeWithCommentsStatement = $mysqlClient->prepare(
    'SELECT *, DATE_FORMAT(c.created_at, "%d/%m/%Y") as comment_date FROM recipes r
...
$averageRatingStatment = $mysqlClient->prepare(
    'SELECT ROUND(AVG(c.review),1) as rating FROM recipes r
    LEFT JOIN comments c on r.recipe_id = c.recipe_id WHERE r.recipe_id = :id');
